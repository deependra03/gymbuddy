        ...
        <div className="space-y-8">
          {Object.entries(grouped).map(([category, exs]) => (
            <div key={category}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {exs.map((ex) => (
                  <div key={ex.id} className="card hover:border-zinc-700 transition-colors group">
                    {/* Thumbnail or placeholder */}
                    <div className="relative -mx-5 -mt-5 mb-4 h-36 bg-zinc-800 rounded-t-2xl overflow-hidden">
                      {ex.thumbnailUrl ? (
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-semibold text-zinc-100 text-sm leading-tight">{ex.title}</h3>
                    ...
                    <div className="flex gap-1 mb-3">
                      <span className="badge bg-zinc-800 text-zinc-400 text-[11px]">{ex.focusArea}</span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-slide-up">
            <div className="sticky top-0 bg-zinc-900 flex items-center justify-between p-6 border-b border-zinc-800">
              <h2 className="text-lg font-bold">{editing ? 'Edit Exercise' : 'Add Exercise'}</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  {thumbnailUrl ? (
                    <img src={thumbnailUrl} className="w-16 h-16 rounded-xl object-cover bg-zinc-800" />
                  ) : (
