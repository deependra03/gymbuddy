...
export default function MemberDietPage() {
  ...
  useEffect(() => {
    ...
    dietApi
      ...
      .then((r) => {
        setPlans(r.data);
        if (r.data.length > 0) setActivePlan(r.data[0]);
      })
    ...
    <div className="page-container space-y-6">
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          <UtensilsCrossed className="w-14 h-14 text-zinc-700 mx-auto mb-3" />
          <p className="text-zinc-300 font-semibold">No diet plan yet</p>
          <p className="text-zinc-500 text-sm mt-1">Your trainer will create a customized nutrition plan for you.</p>
        </div>
      ) : (
        <>
          ...
          {activePlan && (() => {
            ...
            return (
              <div className="space-y-4">
                {/* Plan Header */}
                <div className="card bg-gradient-to-br from-zinc-900 to-zinc-800 border-zinc-700">
                  <h2 className="font-bold text-zinc-100 text-lg">{activePlan.title}</h2>
                  ...
                  {activePlan.notes && (
                    <div className="flex items-start gap-2 mt-4 p-3 bg-zinc-800/60 rounded-xl">
                      <AlertCircle className="w-4 h-4 text-brand-400 shrink-0 mt-0.5" />
                      <p className="text-xs text-zinc-400 leading-relaxed">{activePlan.notes}</p>
                    </div>
                  )}
                </div>
                ...
                {planData?.meals ? (
                      ...
                      <div key={i} className="card hover:border-zinc-700 transition-colors">
                        ...
                        >
                          <div className="flex items-center gap-3 text-left">
                            ...
                            <div>
                              <p className="font-semibold text-zinc-200 text-sm">{meal.name}</p>
                  ...
                  /* Plain text plan */
                  <div className="card">
                    <p className="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap">{activePlan.content}</p>
                  </div>
                )}
              </div>
            );
